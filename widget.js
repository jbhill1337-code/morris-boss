...
// Connect to the same Firebase database
...
const bossRef = firebase.database().ref('frank_corporate_data');

let flashTimeout;

// Listen for updates from viewers clicking on the main website
bossRef.on('value', (snapshot) => {
  ...
  // If health went down, trigger the damage animation automatically!
  if (currentHealth < previousHealth && previousHealth <= currentMaxHealth) {
    const damageTaken = previousHealth - currentHealth;
    triggerHitAnimation(damageTaken);
  }
  
  previousHealth = currentHealth;
});

function triggerHitAnimation(damageAmount) {
  ...
  // Spawn random damage number near the boss
...
function spawnFloatingText(amount) {
  ...
  el.className = `floating-text damage animate-float`;
  
  // Randomize start position near Frank's image
