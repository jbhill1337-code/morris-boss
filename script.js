...
function initSystem() {
  ...
  // Force character images to load
...
const endIntro = () => {
  if (introContainer) {
    ...
    setTimeout(() => { 
      ...
      initSystem();
      load(); // Load save data and show login
    }, 1000);
  }
};
...
// EMERGENCY BLACK SCREEN OVERRIDE
...
/* ══ SAVE / LOAD / CLOCK IN ═════════════════════════════════════════════════ */
function save() {
  if (!isOBS) localStorage.setItem('gwm_v11', JSON.stringify({ c:myCoins, cd:myClickDmg, ad:myAutoDmg, cc:clickCost, ac:autoCost, u:myUser, inv:myInventory, shopMultiplier, synergyCost, rageCost, coinsPerClick, hustleCost }));
}
function load() {
