...
function injectCompactStyles() {
  ...
  style.innerHTML = `
    ...
    #boss-area { display: flex; justify-content: center; align-items: flex-end; min-height: 400px; gap: 20px; transition: all 0.3s ease; }
    #boss-image, #companion-image { height: auto; image-rendering: pixelated; max-width: 100%; }
    @media (min-width: 769px) { #boss-image, #companion-image { width: 380px; } #left-col, #right-col { width: 280px; flex-shrink: 0; } }
    @media (max-width: 768px) { #game-container { flex-direction: column; align-items: center; } #boss-area { min-height: 250px; gap: 10px; } #boss-image, #companion-image { width: 160px; } .column { width: 100% !important; max-width: 400px; } }
  `;
...
window.onPlayerReady = function(event) { if (startIntroBtn) { startIntroBtn.style.display = 'block'; startIntroBtn.onclick = () => { startIntroBtn.style.display = 'none'; if (document.getElementById('yt-player')) document.getElementById('yt-player').style.display = 'block'; if (skipIntroBtn) skipIntroBtn.style.display = 'block'; event.target.playVideo(); }; } };
window.onPlayerStateChange = function(event) { if (event.data === 0) endIntro(); };
window.onYouTubeIframeAPIReady = function () { if (!introContainer || isOBS) return; ytPlayer = new YT.Player('yt-player', { videoId: 'HeKNgnDyD7I', playerVars: { playsinline:1, controls:0, disablekb:1, fs:0, modestbranding:1, rel:0 }, events: { onReady: window.onPlayerReady, onStateChange: window.onPlayerStateChange } }); };
...
/* ══ GAME STATE ═════════════════════════════════════════════════════════════ */
let myCoins = 0, myClickDmg = 2500, myAutoDmg = 0, myUser = '', multi = 1, frenzy = 0;
let clickCost = 10, autoCost = 50, critChance = 0, critCost = 100, autoInterval = 1000, overtimeCost = 200;
let shopMultiplier = 1.0, synergyCost = 150, frenzyGainBonus = 0, rageCost = 75, coinsPerClick = 1, hustleCost = 30;
...
setInterval(() => { const compImg = document.getElementById('companion-image'); if (compImg && !isAnimatingHit) { frameIndex = (frameIndex + 1) % currentCompanion.length; compImg.src = currentCompanion[frameIndex]; } }, 2500);
...
function playHitAnimation() {
  ...
  const bImg = document.getElementById('boss-image'); if (bImg) { const old = bImg.src; bImg.src = daveHitFrames[Math.floor(Math.random() * daveHitFrames.length)]; bImg.classList.add('quick-zoom'); setTimeout(() => { bImg.src = old; bImg.classList.remove('quick-zoom'); }, 250); }
  setTimeout(() => { const cImg = document.getElementById('companion-image'); if (cImg) { cImg.classList.add('quick-zoom'); setTimeout(() => { cImg.classList.remove('quick-zoom'); isAnimatingHit = false; }, 250); } }, 150);
}
function attack(e) {
  ...
  const finalDmg = isCrit ? dmg * 5 : dmg;
...
function startAutoTimer() { if (autoTimer) clearInterval(autoTimer); autoTimer = setInterval(() => { if (myAutoDmg > 0 && bossRef) { const dmg = Math.floor(myAutoDmg * itemBuffMultiplier * shopMultiplier); bossRef.transaction(b => { if (b) b.health -= dmg; return b; }); } }, autoInterval); }
...
function load() {
  const s = localStorage.getItem('gwm_v11'); if (s) {
    const d = JSON.parse(s);
    myCoins = d.c || 0; myClickDmg = d.cd || 2500; myAutoDmg = d.ad || 0; clickCost = d.cc || 10; autoCost = d.ac || 50; myUser = d.u || ''; myInventory = d.inv || {};
...
/* ══ UTILS (POPUPS, LOOT, RICHARD, PHISHING) ════════════════════════════════ */
